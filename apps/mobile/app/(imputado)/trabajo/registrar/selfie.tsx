...
// Verificación facial antes de registrar el sitio de trabajo — evita que
// otra persona haga el registro en lugar del imputado. Mismo dispatcher
// FaceTec/acelerómetro que checkin/selfie.tsx, según el toggle de la org.
export default function TrabajoSelfieScreen() {
...
// ── FaceTec: el veredicto queda server-side en facetec_sessions
// (checkin_id NULL, kind='auth') — register-work-location lo lee ahí. ────────
function FacetecSelfie() {
  const router = useRouter()
  const { profile } = useAuth()
  const { setFaceVerified } = useWorkLocationStore()
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (!result.livenessPassed) {
        ...
        setMsg('No se completó la verificación facial. Debes mostrar tu rostro en vivo para continuar.')
        setStatus('failed')
        return
      }
      setFaceVerified()
      router.push('/(imputado)/trabajo/registrar/gps')
    } catch (e: any) {
      runningRef.current = false
...
// ── Acelerómetro (mismo challenge del check-in, sin FaceTec) ────────────────
...
function AccelerometerSelfie() {
  const router = useRouter()
  const { setSelfie } = useWorkLocationStore()
    ...
    return (
      <View style={styles.container}>
        <View style={styles.center}>
          ...
          <Text style={styles.subtitle}>Para verificar tu identidad, Arraigo necesita acceso a tu cámara frontal.</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Mira a la cámara e inclina el teléfono siguiendo las instrucciones.</Text>
      </View>
      <View style={styles.cameraContainer}>
        <CameraView ref={cameraRef} style={styles.camera} facing="front">
          <View style={styles.ovalGuide} />
          <View style={styles.arrowRow}>
            <Text style={[styles.arrow, firstDone ? styles.arrowDone : styles.arrowPending]}>{order[0] === 'left' ? '←' : '→'}</Text>
            <Text style={[styles.arrow, secondDone ? styles.arrowDone : styles.arrowPending]}>{order[1] === 'left' ? '←' : '→'}</Text>
          </View>
        </CameraView>
      </View>
...
const styles = StyleSheet.create({
  ...
  cameraContainer: { flex: 1, margin: 24, borderRadius: 24, overflow: 'hidden', borderWidth: 2, borderColor: '#2563eb' },
  camera: { flex: 1 },
  ovalGuide: {
  ...
  arrowRow: { position: 'absolute', bottom: 20, left: 0, right: 0, flexDirection: 'row', justifyContent: 'center', gap: 40 },
