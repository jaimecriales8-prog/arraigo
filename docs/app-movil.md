...
**Pendiente:**
- **FaceTec** — puente nativo Kotlin ya escrito y con integración real (`android/app/src/main/java/co/arraigo/app/facetec/`: `FacetecModule.kt`, `FacetecSessionProcessor.kt`, `FacetecPackage.kt`), portado 1:1 desde `ios/Arraigo/Facetec/*.swift` usando de referencia la SampleApp oficial de FaceTec (SDK v10.1.9, `.aar` en `android/app/libs/`). Todavía sin compilar/probar contra un dispositivo real — falta Android Studio instalado. Detalle completo en `android/FACETEC_SETUP_ANDROID.md`. Mientras tanto, el toggle `facetecEnabled` por organización permite operar con el liveness por acelerómetro.
