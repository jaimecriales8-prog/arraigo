...
- **Reiniciar enrolamiento facial**: cuando FaceTec rechaza un re-enrolamiento (`"An enrollment already exists for this externalDatabaseRefID"` — pasa cuando un enrolamiento queda a medias por batería/red/cámara durante el onboarding real, no solo en pruebas), el técnico ya no tenía forma de resolverlo sin crear un imputado nuevo. Nuevo edge function `reset-facetec-enrollment` — solo el técnico asignado (o `super_admin`), y solo mientras el caso sigue en `onboarding` (una vez activo, cambiar la referencia facial necesita un control más estricto). Botón "Reiniciar enrolamiento" en `identidad.tsx`, visible solo tras un fallo.
  → `supabase/functions/reset-facetec-enrollment/`, `apps/mobile/app/(tecnico)/onboarding/[caseId]/identidad.tsx`

**Bug encontrado el 2026-07-30 (vía video del técnico) y corregido:** el endpoint asumido originalmente (`DELETE .../enrollment-3d`) **no existía** — devolvía 404 "No route found" en cada intento, así que el botón nunca funcionó y el técnico terminaba en el "Error Crítico / Sesión Finalizada" nativo de FaceTec. Ninguna documentación pública de FaceTec es accesible sin cuenta de desarrollador (todo detrás de login). Se probó empíricamente contra la API real (con el caso ya bloqueado de Alexandra Gómez) hasta encontrar el endpoint correcto: `POST /api/v4/biometrics/3d-db/delete` con body `{"identifier": "<externalDatabaseRefID>"}` (no `DELETE`, no `externalDatabaseRefID` como nombre de campo) — confirmado `success:true` contra la API real, dos veces (idempotente). **Pendiente:** el usuario aún no ha vuelto a probar el flujo completo de enrolamiento con el fix aplicado — falta esa verificación final antes de dar el punto por cerrado del todo.
  → `supabase/functions/reset-facetec-enrollment/index.ts` (commit `669bc36`)
...
### 28. Aviso por WhatsApp al imputado tras 2 verificaciones seguidas sin completar (2026-08-19)
Un paso antes del escalamiento a alerta crítica que ya existe a las 3 seguidas (`check_case_escalation()`, punto 6) — mismo cálculo de racha (`streak_len=2` en vez de 3), pero con un canal directo al imputado por WhatsApp en vez de solo una alerta en el panel. Vía **SendPulse** (WhatsApp Business), reutilizando el cliente ya probado en producción en otro proyecto del usuario (GeoDataVoice, mismo bot ID `6a2d7fd90abdaad9e40f5ae8`) — mismo patrón OAuth `client_credentials` + `sendTemplateByPhone` + normalización de teléfono a Colombia, portado tal cual.

La detección (`find_pending_whatsapp_warnings()`) es set-based (sin loop fila por fila, lección de la ronda de escala), con dedup por `checkin_id` en la tabla nueva `whatsapp_notifications` para no reenviar mientras la racha sigue. El envío corre en un edge function (`send-whatsapp-warnings`) programado como **Supabase Edge Function cron job** (Database → Cron Jobs, tipo nativo — no `pg_net`/SQL crudo con el service-role key embebido, que fue el primer enfoque considerado y descartado por seguridad) cada 15 min.

Desplegado y verificado de punta a punta contra producción real (secrets copiados de GeoDataVoice vía `vercel env pull`, RPC de detección probado con un caso real — Jaime Criales, con 2 fallidas seguidas genuinas — y el edge function completo invocado manualmente: encontró el caso, intentó el envío, y falló exactamente con el error esperado `"Template does not exist"`, confirmando que todo el pipeline funciona salvo la plantilla). **Pendiente:** el usuario debe crear la plantilla `aviso_dos_fallidas` en SendPulse (categoría Utility, 1 variable, texto: *"Hola {{1}}, tu proceso de seguimiento electrónico registra 2 verificaciones seguidas sin completar. Por favor realiza tu próxima verificación a tiempo."*) y esperar la aprobación de Meta — sin eso, el envío real sigue fallando con ese mismo error (comportamiento esperado, no un bug).

→ `supabase/migrations/20260803_037_whatsapp_avisos_dos_fallidas.sql`, `supabase/functions/_shared/sendpulse.ts`, `supabase/functions/send-whatsapp-warnings/`

**Fuera de alcance:** avisar también al staff por WhatsApp, reintentos automáticos si falla el envío, configurar el texto/plantilla desde el panel.
